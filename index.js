...
async function run() {
  ...
  app.get("/product", async (req, res) => {
    ...
    const result = await ProductCollection.find({
      ...
      price: {
        ...
        $lte: maxprice ? maxprice : maxPrice,
      },
    }).toArray();
  ...
  app.post("/payment", verifyJWT, async (req, res) => {
    ...
    const data = {
      ...
      success_url: `https://yourshop-green.vercel.app/payment/success/${tran_id}`,
      fail_url: `https://yourshop-green.vercel.app/payment/fail/${tran_id}`,
      cancel_url: "https://yourshop-green.vercel.app/cancel",
      ipn_url: "https://yourshop-green.vercel.app/ipn",
    ...
    app.post("/payment/success/:tranID", async (req, res) => {
      ...
      const deleteCard = await addcardCollection.deleteOne({
      ...
      if (result.matchedCount > 0 && deleteCard?.deletedCount > 0) {
        res.redirect(
          `https://logecgi-com.vercel.app/payment/success/${req.params.tranID}`
        );
      }
    });
    ...
    app.post("/payment/fail/:tranID", async (req, res) => {
      ...
      if (result.deletedCount > 0) {
        res.redirect(
          `https://logecgi-com.vercel.app/paymentfail/fail/${req.params.tranID}`
        );
      }
    });
  });
